    ...
    with open(csv_all_path, mode='w', newline='') as csvfile:
        ...
        for num_pieces in range(1, 8):
            ...
            for idx in range(samples_per_piece):
                ...
                # For all piece counts, first create a 7-piece tangram
                # and then randomly remove pieces until desired count remains
                ...
                for (nm, poly) in shapes_selected[1:]:
                    ok, newp = try_place_tan(placed_polys, poly, max_attempts=2048)
                    if not ok:
                        success = False
                        break
                    placed_polys.append(newp)
                
                # If successful, randomly remove pieces until desired count remains
                if success and within_bounds(placed_polys):
                    # Keep removing pieces randomly until we have the desired number
                    while len(placed_polys) > num_pieces:
                        # Randomly select a piece to remove
                        piece_to_remove = random.randrange(len(placed_polys))
                        placed_polys.pop(piece_to_remove)
                else:
                    # If 7-piece generation failed, fall back to standard generation
